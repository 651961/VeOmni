...
from veomni.data.diffusion.dataset import build_qwen_image_edit_2511_tensor_dataset
...
def main():
    ...
    if args.data.data_type == "diffusion":
        train_dataset = build_qwen_image_edit_2511_tensor_dataset(
    ...
    if args.train.save_initial_model:
        if args.train.global_rank == 0:
            ...
            save_model_weights(args.train.output_dir, model.state_dict(), model_assets=model_assets)
    ...
    if args.train.load_checkpoint_path:
        ...
        for key in ["lr", "betas", "eps", "weight_decay", "initial_lr"]:
            if key not in optimizer.param_groups[0]:
                optimizer.param_groups[0][key] = optimizer.param_groups[1][key]
    ...
    for epoch in range(start_epoch, args.train.num_train_epochs):
        ...
        for _ in range(start_step, args.train.train_steps):
            ...
            for micro_batch in micro_batches:
                environ_meter.add(micro_batch, model_type="qwen_image_edit_2511")

                latents = micro_batch["latents"].to(model.device)
                edit_latents = [el.to(model.device) for el in micro_batch["edit_latents"]]
                height = micro_batch["height"]
                width = micro_batch["width"]
                prompt_emb = micro_batch["prompt_emb"].to(model.device)
                prompt_emb_mask = micro_batch["prompt_emb_mask"].to(model.device)
